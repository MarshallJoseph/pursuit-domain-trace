...
def output_images(w, h, trace, index, group):
    ...
    if group == 1:
        ...
        img.save("trace" + str(index) + "-xy.png")
    elif group == 3:
        img.save("trace" + str(index) + "x-y.png")
    elif group == 4:
        img.save("trace" + str(index) + "-x-y.png")
    elif group == 5:
        img.save("trace" + str(index) + "yx.png")
    elif group == 6:
        img.save("trace" + str(index) + "-yx.png")
    elif group == 7:
        img.save("trace" + str(index) + "y-x.png")
    elif group == 8:
        img.save("trace" + str(index) + "-y-x.png")
...
def open_trace(i, group):
    ...
    if group == 1:
        ...
        with open("files/trace" + str(i) + "-xy.txt") as f:
            lines = [line.rstrip() for line in f]
    elif group == 3:
        with open("files/trace" + str(i) + "x-y.txt") as f:
            lines = [line.rstrip() for line in f]
    elif group == 4:
        with open("files/trace" + str(i) + "-x-y.txt") as f:
            lines = [line.rstrip() for line in f]
    elif group == 5:
        with open("files/trace" + str(i) + "yx.txt") as f:
            lines = [line.rstrip() for line in f]
    elif group == 6:
        with open("files/trace" + str(i) + "-yx.txt") as f:
            lines = [line.rstrip() for line in f]
    elif group == 7:
        with open("files/trace" + str(i) + "y-x.txt") as f:
            lines = [line.rstrip() for line in f]
    elif group == 8:
        with open("files/trace" + str(i) + "-y-x.txt") as f:
            lines = [line.rstrip() for line in f]
